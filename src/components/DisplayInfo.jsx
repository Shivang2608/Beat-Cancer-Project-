...
const DisplayInfo = () => {
  ...
  // Effect 1: Sirf data fetch karne ke liye
  useEffect(() => {
    if (userEmail) {
      fetchUserRecords(userEmail);
    }
  }, [userEmail, fetchUserRecords]); // Yeh stable hai

  // Effect 2: Metrics calculate karne ke liye
  // Hum 'records' object ko string mein convert karke depend karenge
  const recordsString = JSON.stringify(records);

  useEffect(() => {
    console.log("Records updated:", records); // Yeh ab loop nahi karega
  ...
  }, [recordsString]); // Ab yeh tabhi chalega jab records ka *content* badlega
